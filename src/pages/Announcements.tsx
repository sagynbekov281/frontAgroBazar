  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-bold text-2xl">Сатып алуу жарыялары</h1>
          <p className="text-muted text-sm mt-0.5">Эмне керектигин жаз — фермерлер өздөрү сунуш жасайт</p>
        </div>
        <button onClick={() => user ? setShowForm(true) : navigate('/login')} className="btn btn-accent gap-2">
          <Plus size={18} /> Жарыя жаз
        </button>
      </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          <div className="bg-card rounded-2xl p-6 w-full max-w-lg shadow-xl" onClick={e => e.stopPropagation()}>
            <h2 className="font-bold text-lg mb-4">Жаңы сатып алуу жарыясы</h2>
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="label">Көлөм *</label><input required type="number" className="input text-sm" value={form.qty} onChange={e => setForm(f => ({ ...f, qty: e.target.value }))} /></div>
                <div><label className="label">Бирдик</label><input className="input text-sm" value={form.unit} onChange={e => setForm(f => ({ ...f, unit: e.target.value }))} /></div>
              </div>
              <div><label className="label">Сүрөттөмө</label><textarea rows={3} className="input text-sm" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} /></div>
              <div><label className="label">Дедлайн</label><input type="date" className="input text-sm" value={form.deadline} onChange={e => setForm(f => ({ ...f, deadline: e.target.value }))} /></div>
              <div className="flex gap-2 pt-2">
                <button type="button" onClick={() => setShowForm(false)} className="btn btn-outline flex-1">Жокко чыгаруу</button>
                <button disabled={saving} className="btn btn-primary flex-1">{saving ? 'Сакталууда...' : 'Жарыялоо'}</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {announcements.length === 0 ? (
            <div className="text-center py-20">
              ...
              <div className="font-semibold">Жарыялар жок</div>
              <div className="text-sm text-muted mt-1">Биринчи болуп жарыя жазыңыз</div>
            </div>
          ) : announcements.map(a => (
            <div key={a.id} className="card p-5 hover:shadow-md transition flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-3 text-sm text-muted">
                  ...
                  <span className="flex items-center gap-1"><MapPin size={13} /> {a.region || 'Бардык аймак'}</span>
              ...
              <div className="shrink-0 text-right">
                <div className="text-sm font-semibold text-primary-700">{a.offerCount} сунуш</div>
                <div className="text-xs text-muted">{a.authorName}</div>
                <button className="btn btn-outline text-xs px-3 py-1.5 mt-2 gap-1">Сунуш жаз <ChevronRight size={13} /></button>
              </div>
            </div>
          ))}
        </div>
